    ...
    with codecs.open("log", "a", encoding='utf-8') as file1:
        ...
        if text == '1':
            ...
            bot.send_message(chat_id=msg.from_user.id,
                             text='Введите выражение с пробелами между числами и знаками')
        elif text == '2':
            bot.register_next_step_handler(msg, compl_calc)
            bot.send_message(chat_id=msg.from_user.id,
                             text='Введите выражение с пробелами между числами и знаками, но без пробела внутри комплексного числа')
...
def float_calc(msg: types.Message):
    ...
    if lst[1] == '+':
        bot.send_message(chat_id=msg.from_user.id,
                         text=f'Результат сложения {float(lst[0]) + float(lst[2])}')
    elif lst[1] == '-':
        bot.send_message(chat_id=msg.from_user.id,
                         text=f'Результат вычитания {float(lst[0]) - float(lst[2])}')
    elif lst[1] == '*':
        bot.send_message(chat_id=msg.from_user.id,
                         text=f'Результат умножения {float(lst[0]) * float(lst[2])}')
    elif lst[1] == '/':
        if lst[2] == '0':
            bot.send_message(chat_id=msg.from_user.id,
                             text='Деление на 0 недопустимо')
            os._exit()
            bot.send_message(chat_id=msg.from_user.id,
                             text=f'Результат деления {float(lst[0]) / float(lst[2])}')

        else:
            bot.send_message(chat_id=msg.from_user.id, text='Вы прислали: ' + msg.text +
                             ', а должны были "число, пробел, знак действия, пробел, число"')
...
def compl_calc(msg: types.Message):
    ...
    if lst[1] == '+':
        ...
        if '1j' in result:
            result.replace('1j', 'j')
        bot.send_message(chat_id=msg.from_user.id,
                         text=f'Результат сложения {result}')
    if lst[1] == '-':
        result = str(a_complex - b_complex)
        if '1j' in result:
            result.replace('1j', 'j')
        bot.send_message(chat_id=msg.from_user.id,
                         text=f'Результат вычитания {result}')
    if lst[1] == '*':
        result = str(a_complex * b_complex)
        if '1j' in result:
            result.replace('1j', 'j')
        bot.send_message(chat_id=msg.from_user.id,
                         text=f'Результат умножения {result}')
    if lst[1] == '/':
        result = str(a_complex / b_complex)
        if '1j' in result:
            result.replace('1j', 'j')
        bot.send_message(chat_id=msg.from_user.id,
                         text=f'Результат деления {result}')
